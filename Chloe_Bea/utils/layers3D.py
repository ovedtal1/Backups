...
from utils.transforms_modl import center_crop
...
class SeparableConv3d(nn.Module):
    ...
    def __init__(self, in_chans, out_chans, kernel_size, spatial_chans=None, act_type='relu'):
        ...
        spatial_conv = nn.Conv3d(in_chans, spatial_chans, kernel_size=sp_kernel_size, padding=sp_pad_size)
        temporal_conv = nn.Conv3d(spatial_chans, out_chans, kernel_size=t_kernel_size, padding=t_pad_size)
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_chans, out_chans, kernel_size, drop_prob, 
                 conv_type='conv3d', act_type='relu', norm_type='none'):
        """
        Args:
            in_chans (int): Number of channels in the input.
            out_chans (int): Number of channels in the output.
...
#         print(conv_type=='conv3d')
        if conv_type == 'conv3d':
            convolution = nn.Conv3d(in_chans, out_chans, kernel_size=kernel_size, padding=1)
#             print("is conv3d")
        else:
            convolution = SeparableConv3d(in_chans, out_chans, kernel_size=kernel_size)
#             print("is conv2p1d")
...
#             sys.exit()
    ...
    def __repr__(self):
        return f'ConvBlock3D(in_chans={self.in_chans}, out_chans={self.out_chans}, ' \
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_chans, out_chans, kernel_size, drop_prob, conv_type='conv3d'):
        """
        Args:
            in_chans (int): Number of channels in the input.
            out_chans (int): Number of channels in the output.
        ...
        self.layers = nn.Sequential(
            ConvBlock(in_chans, out_chans, kernel_size, drop_prob,conv_type),
            ConvBlock(out_chans, out_chans, kernel_size, drop_prob,conv_type)
        )
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_resblocks, in_chans, chans, kernel_size, drop_prob, circular_pad=True,conv_type='conv3d'):
        """
        ...
        self.res_blocks = nn.ModuleList([ResBlock(in_chans, chans, kernel_size, drop_prob,conv_type)])
        for _ in range(num_resblocks-1):
            self.res_blocks += [ResBlock(chans, chans, kernel_size, drop_prob,conv_type)]
