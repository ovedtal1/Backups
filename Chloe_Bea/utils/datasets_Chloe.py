...
class SliceData(Dataset):
    ...
    def _find_examples(self, root):
        ...
        file_pattern = os.path.join(root, '**', 'LF*reg.nii')
        ...
        for file in files:
            ...
            if 'LF' in basename:
                ...
                week_number = ''.join(filter(str.isdigit, basename.split('LF')[1].split('reg')[0]))
                #print(week_number)
                #print(file)
                reference_pattern = f"HF*regLF{week_number}.nii"
    ...
    def __getitem__(self, i):
        ...
        target = torch.from_numpy(data[:,:-5,:])
        target = F.pad(target, (4, 5, 0, 0), mode='constant', value=0).numpy() 
        target = target.squeeze(0)
        
        ## For hard augmenting
        #print(f'Targert 1st: {target.shape}')
        
        random_angle = random.uniform(-15, 15) # was 90
        target = rotate(target, angle=random_angle, axes=(0, 1), reshape=False)
        random_shift_x = random.uniform(-5, 5)  # Adjust the shift range as needed # was 20
        random_shift_y = random.uniform(-5, 5)  # Adjust the shift range as needed
        target = shift(target, shift=[random_shift_x, random_shift_y])
        
        #print(f'Targert 2nd: {target.shape}')
        
        random_phase = torch.angle(T.random_map((1,155,155), 'cpu',kspace_radius_range=(0.001, 0.001))) *0
        ...
        ref_target = torch.from_numpy(ref_data[:,:-5,:] )
        ref_target = F.pad(ref_target, (4, 5, 0, 0), mode='constant', value=0).numpy() 
