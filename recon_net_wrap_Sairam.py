...
class BilinearPoolingFusionNet(nn.Module):
    def __init__(self):
        ...
        self.param1 = nn.Parameter(torch.normal(0.9, 0.0, size=(256,)))
        self.param2 = nn.Parameter(torch.normal(0.1, 0.005, size=(256,)))  
        self.epsilon = 1e-6
...
class ViTfuser(nn.Module):
    def __init__(self, net, epsilon=1e-6):
        ...
        cp_ref = torch.load('./L2_checkpoints_ViT_mirror/model_30.pt', map_location=self.device)
        self.recon_net.load_state_dict(cp['model_state_dict'])
        #self.recon_net_ref.load_state_dict(cp_ref['model'])
        ...
        # Low Resolution
        self.param1 = nn.Parameter(torch.normal(1, 0.00, size=(256,)))
        self.param2 = nn.Parameter(torch.normal(0, 0.00, size=(256,)))
    ...
    def forward(self, img,ref): #,ref
        ...
        # Low Resolution
        features_comb = features_comb.reshape(features_flat.shape[0], 256, 1024)
        ...
        # Low Resolution 
        head_out_img = self.recon_net.net.seq2img(head_out, (160, 160))
