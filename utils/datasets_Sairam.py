...
class SliceData(Dataset):
    ...
    def __getitem__(self, i):
        ...
        target = data[:, :]
        ## Fix resolution
        target = torch.from_numpy(resize(np.abs(target[0,:,:]), (155, 155), anti_aliasing=True))

        target = target.squeeze(0)
        ## For hard augmenting
        #print(f'Targert 1st: {target.shape}')
        
        random_angle = random.uniform(-90, 90) # was 90
        ...
        ref_target = ref_data[:, :] 
        ## Fix resolution
        ref_target = resize(np.abs(ref_target[0,:,:]), (155, 155), anti_aliasing=True) * (torch.exp(1j * random_phase)).numpy() 
